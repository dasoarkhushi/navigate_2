...
// 🔹 Mutual fund identifiers (codes/IDs your backend understands)
const mfCodes = [
  "120503", // SBI Bluechip Fund - Direct Growth
  "119551", // HDFC Top 100 - Direct Growth
  "118574", // Kotak Emerging Equity - Direct Growth
  "102885", // HDFC Mid-Cap Opportunities
  "101206", // Nippon India Small Cap
  "125354", // SBI Small Cap
  "135781", // UTI Nifty 50 Index
  "135779", // HDFC Nifty 50 Index
  "145678", // SBI Equity Hybrid
  "110458"  // Axis Long Term Equity (ELSS)
];
...
const Insight = () => {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      // Backend: make fetchInsights handle MF codes instead of stock symbols
      const data = await fetchInsights(mfCodes);
      setFunds(Array.isArray(data) ? data : []);
    } catch (e) {
      console.error("Error fetching MF insights:", e);
      setFunds([]);
    }
    setLoading(false);
  };
    ...
    return (
      <Box m="20px">
        <Header
          ...
          subtitle="Live Mutual Fund Trends & Performance (NAVigate)"
        />
        <CircularProgress sx={{ mt: 4 }} />
      </Box>
    );
    ...
    return (
      <Box m="20px">
        <Header
          ...
          subtitle="Live Mutual Fund Trends & Performance (NAVigate)"
        />
        <Typography mt={4} color={colors.grey[300]}>
          No mutual fund data available. Please check the insights API or sample
          data.
        </Typography>
      </Box>
    );
  ...
  const sentimentData = [
    {
      id: "Bullish",
      label: "Bullish",
      value: funds.filter((f) => (f.changePercent || 0) > 0).length,
      color: "hsl(140, 70%, 50%)"
    },
    {
      id: "Bearish",
  ...
  // Line chart for top 5 funds by NAV
  const lineData = [
    {
      id: "Top 5 MFs",
      data: sorted.slice(0, 5).map((f) => ({
        x:
          f.schemeShortName ||
          f.schemeName ||
          f.code ||
      ...
      >
        ...
        >
          ...
          <Typography
            ...
            mt="10px"
          >
            +{(gainer?.changePercent ?? 0).toFixed(2)}%
          </Typography>
          <Typography color={colors.grey[100]} mt="10px">
            NAV: ₹{(gainer?.nav ?? gainer?.price ?? 0).toFixed(2)}
          </Typography>
        </Box>
        ...
        >
          ...
          <Typography
            ...
            mt="10px"
          >
            {(loser?.changePercent ?? 0).toFixed(2)}%
          </Typography>
          <Typography color={colors.grey[100]} mt="10px">
            NAV: ₹{(loser?.nav ?? loser?.price ?? 0).toFixed(2)}
          </Typography>
        </Box>
